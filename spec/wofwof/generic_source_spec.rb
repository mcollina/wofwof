...
describe GenericSource do
  ...
  before(:each) do
    ...
    @klass = mock "Class" 
    @instance = GenericSource.build(@klass).new(@path_handler)
  end
  ...
  it "should correctly create two page nodes if the path handler extracts two paths" do
    ...
    @klass.should_receive(:new).with(context, first_path, @path_handler).and_return(first_node)

    second_node = mock "SecondNode"
    @klass.should_receive(:new).with(context, second_path, @path_handler).and_return(second_node)
